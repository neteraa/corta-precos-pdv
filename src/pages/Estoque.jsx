...
export default function Estoque() {
  const { products, upsertProduct, addLot } = useStore()
  ...
  const scanEntrada = useCallback((raw) => {
    ...
    setEntradaList(prev => {
      // Incrementa o 1º lote já aberto do produto (sem data ou mesma data)
      const idx = prev.findIndex(e => e.product.id === found.id)
      if (idx >= 0) {
        const next = [...prev]; next[idx] = { ...next[idx], qty: next[idx].qty + 1 }; return next
      }
      // Novo lote
      return [{ rowId: `row_${Date.now()}`, product: found, qty: 1, expiryDate: found.expiryDate || '' }, ...prev]
    })
    flashFeedback(`✅ ${found.name}`)
  }, [products])

  // Adiciona uma linha extra para o mesmo produto (lote diferente)
  const addExtraLot = useCallback((productId) => {
    const prod = products.find(p => p.id === productId)
    if (!prod) return
    setEntradaList(prev => [...prev, { rowId: `row_${Date.now()}`, product: prod, qty: 1, expiryDate: '' }])
  }, [products])

  const confirmEntrada = () => {
    entradaList.forEach(({ product, qty, expiryDate }) => {
      addLot(product.id, { qty, expiryDate })
    })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="card w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl animate-pop">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 min-h-[160px]">
              {entradaList.length === 0 ? (
              ...
              ) : entradaList.map(({ rowId, product, qty, expiryDate }) => {
                ...
                // Número de lotes deste produto já na lista
                const lotCount  = entradaList.filter(e => e.product.id === product.id).length
                const lotIdx    = entradaList.filter(e => e.product.id === product.id).findIndex(e => e.rowId === rowId)
                return (
                  <div key={rowId} className="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-2.5">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-sm text-gray-900 truncate">
                          {product.name}
                          {lotCount > 1 && <span className="ml-1.5 text-[10px] font-black bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">Lote {lotIdx+1}</span>}
                        </div>
                        <div className="text-xs text-gray-400 mt-0.5">
                          Estoque atual: <strong>{product.stock}</strong> {product.unit||'un'} · este lote: <span className="font-bold text-green-600">+{qty}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <button onClick={() => setEntradaList(l => l.map(e => e.rowId === rowId ? { ...e, qty: Math.max(1, e.qty-1) } : e))}
                        ...
                        <input type="number" min="1" value={qty}
                          onChange={e => { const v = Math.max(1, parseInt(e.target.value)||1); setEntradaList(l => l.map(x => x.rowId === rowId ? {...x, qty:v} : x)) }}
                        ...
                        <button onClick={() => setEntradaList(l => l.map(e => e.rowId === rowId ? {...e, qty: e.qty+1} : e))}
                        ...
                        <button title="Novo lote (data diferente)" onClick={() => addExtraLot(product.id)}
                          className="w-7 h-7 rounded-lg bg-amber-100 hover:bg-amber-200 flex items-center justify-center text-amber-700 text-xs font-black">L+</button>
                        <button onClick={() => setEntradaList(l => l.filter(e => e.rowId !== rowId))}
                    ...
                    <div className="flex items-center gap-2 px-4 pb-2.5">
                      ...
                      <input type="date" value={expiryDate||''}
                        onChange={e => setEntradaList(l => l.map(x => x.rowId === rowId ? {...x, expiryDate: e.target.value} : x))}
            ...
            <div className="p-4 border-t border-gray-100 flex gap-2 flex-shrink-0">
              <div className="flex-1 bg-gray-50 rounded-xl px-4 py-2 text-center">
                ...
                <div className="font-black text-gray-900">{entradaList.length} {entradaList.length === 1 ? 'lote' : 'lotes'} · {entradaList.reduce((s,e)=>s+e.qty,0)} un.</div>
              </div>
